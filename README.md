...
> **许可说明：** 本项目允许非商业使用、运行、修改和再分发。分发原版或修改版时必须附带许可证并保留版权声明；任何商业用途均需版权所有者事先书面授权。详见 [LICENSE](LICENSE)。
...
本项目采用 **PolyForm Noncommercial License 1.0.0**。你可以将源码用于个人、学习、研究及其他非商业目的，也可以在非商业范围内运行、修改、制作衍生作品和再分发。

使用和分发时必须：

- 向接收者同时提供完整许可证或许可证官方链接；
- 保留 `Required Notice: Copyright (c) 2026 jiongjiong123441.` 版权声明；
- 确保使用、修改和分发均属于许可证允许的非商业目的。

商业使用、商业部署、收费服务或具有预期商业应用的使用，需要另行取得版权所有者的书面授权。完整条款及免责声明请参阅 [LICENSE](LICENSE)。由于该许可证限制商业用途，它属于非商业源码开放许可证，而不是 OSI 认证的开源许可证。
