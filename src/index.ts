...
app.get('/admin/devices/:id/agent-install', async (c) => {
  ...
  const body = `<div class="page-header"><div><p class="section-code">WINDOWS AGENT</p><h2>Windows 客户端安装信息</h2><p>${sanitizePlainText(device.name, 120)} · 访问码有效 15 分钟且只能使用一次。</p></div><a href="/admin/device-agent-bindings" class="button button-secondary">返回绑定设备</a></div><div class="panel"><h3>6 位 Windows 客户端访问码</h3><p>客户端会先读取 BIOS 序列号自动绑定；如果没有匹配序列号，才使用下面的访问码。</p><div style="font-size:42px;letter-spacing:12px;font-weight:700;margin:28px 0;color:var(--accent,#2563eb);"><code>${code}</code></div><div class="record-actions"><button class="button button-primary" type="button" id="copy-agent-code">复制 6 位访问码</button><a class="button button-primary" href="/downloads/RentDeviceAgent-Setup.exe">下载 Windows 安装程序（EXE）</a></div><p>访问码剩余：<strong id="agent-code-countdown" data-expires-at="${expiresAtMs}">15 分钟</strong>。安装程序会安装到系统目录并注册 Windows Service。</p><p class="page-notification page-notification--warning">访问码只显示这一次。关闭页面后无法恢复，只能重新生成。绑定成功后访问码立即失效。</p></div><script>(()=>{const b=document.getElementById('copy-agent-code'),c=document.getElementById('agent-code-countdown'),expires=Number(c?.dataset.expiresAt||0);b?.addEventListener('click',async()=>{await navigator.clipboard.writeText('${code}');b.textContent='已复制';setTimeout(()=>b.textContent='复制 6 位访问码',1600);});const tick=()=>{const left=Math.max(0,expires-Date.now());if(!c)return;if(!left){c.textContent='已过期';c.style.color='#b42318';return;}const total=Math.ceil(left/1000),minutes=Math.floor(total/60),seconds=total%60;c.textContent=minutes?minutes+' 分 '+String(seconds).padStart(2,'0')+' 秒':seconds+' 秒';setTimeout(tick,1000);};tick();})();</script>`
