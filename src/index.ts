...
app.use('*', async (c, next) => {
  ...
  // 营销官网（rent-web）是独立部署、独立域名，会跨站调用这个公开下单接口。
  // 只放行 PUBLIC_WEB_ORIGIN 指定的来源，其余仍按同源校验拒绝；Turnstile 在 action 内再做人机校验。
  ...
  if (isPublicRentalRequest && (c.req.method === 'POST' || c.req.method === 'OPTIONS')) {
    const origin = c.req.header('Origin') || ''
    if (!publicWebOrigin || origin.replace(/\/$/, '') !== publicWebOrigin) {
...
// 公开下单接口：营销官网 rent-web 提交租期信息 -> 创建 order+contract -> 返回 /contract/sign 链接。
// CORS / 来源校验 / 限流在上面的全局中间件里处理；人机校验在 action 内（Turnstile）。
app.post('/public/rental-request', async (c) => {
  ...
  try {
    ...
    body = contentType.includes('application/json')
      ...
      : (Object.fromEntries(Object.entries(await c.req.parseBody()).map(([k, v]) => [k, String(v)])) as Record<string, string>)
  } catch {
