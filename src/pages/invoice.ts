...
import { buildLayout, formatCurrency, generateReferenceNumber, getContractByOrderId, getOrderById, getUserById, getSystemSettings, isContractFinalized } from '../site'
...
export async function renderInvoice(c: Context, user: any, orderId: string, printMode = false) {
  ...
  for (const invoice of invoices) {
    if (invoice.type === 'credit_note' && /^CN-INV-/.test(String(invoice.invoice_number || ''))) {
      const creditNoteNumber = generateReferenceNumber('CN')
      await c.env.RENT.prepare("UPDATE invoices SET invoice_number = ? WHERE id = ? AND type = 'credit_note'").bind(creditNoteNumber, invoice.id).run()
      invoice.invoice_number = creditNoteNumber
    }
  }
  ...
  const docStyles = `.official-document{width:min(100%,794px);min-height:1123px;margin:0 auto 20px;padding:72px 64px;background:#fff;color:#172033;border:1px solid #d7dde5;box-shadow:0 12px 30px rgba(15,23,42,.12);box-sizing:border-box}.official-document__header{display:flex;justify-content:space-between;gap:32px}.official-document__header h1{margin:0 0 8px;font-size:26px}.official-document__header p{margin:3px 0;color:#667085;font-size:13px}.official-document__eyebrow{letter-spacing:.16em;font-size:11px!important;font-weight:700;color:#2457d6!important}.official-document__title{text-align:right}.official-document__title h2{margin:0 0 12px;font-size:22px}.official-document__title p{margin:5px 0}.official-document__rule{height:3px;background:#2457d6;margin:30px 0}.official-document__meta{display:grid;grid-template-columns:repeat(3,1fr);gap:24px;margin-bottom:34px}.official-document__meta span{display:block;text-transform:uppercase;letter-spacing:.08em;font-size:10px;color:#667085;margin-bottom:8px}.official-document__meta strong{display:block;font-size:14px}.official-document__meta p{margin:5px 0;color:#667085;font-size:12px}.official-document__table{width:100%;border-collapse:collapse;margin-bottom:26px}.official-document__table th{text-align:left;background:#f3f6fa;color:#526071;font-size:11px;text-transform:uppercase;letter-spacing:.06em;padding:13px}.official-document__table td{border-bottom:1px solid #e5e9ef;padding:16px 13px;font-size:13px}.official-document__table th:nth-child(2),.official-document__table th:last-child,.official-document__table td:nth-child(2),.official-document__table td:last-child{text-align:right}.official-document__table small{display:block;color:#667085;margin-top:5px}.official-document__totals{display:flex;justify-content:space-between;gap:32px;border-top:1px solid #dbe2ea;padding-top:20px}.official-document__totals p{font-size:11px;color:#667085}.official-document__totals dl{min-width:260px;margin:0}.official-document__totals dl div{display:flex;justify-content:space-between;padding:6px 0}.official-document__totals dt{color:#667085}.official-document__totals dd{margin:0}.official-document__grand-total{border-top:2px solid #172033;margin-top:8px;padding-top:12px!important;font-size:18px;font-weight:700}.official-document__footer{display:flex;justify-content:space-between;border-top:1px solid #e5e9ef;margin-top:38px;padding-top:14px;color:#98a2b3;font-size:10px}@media(max-width:700px){.official-document{width:100%;min-height:0;padding:28px 20px}.official-document__header,.official-document__totals{display:block}.official-document__title{text-align:left;margin-top:24px}.official-document__meta{grid-template-columns:1fr}.official-document__totals dl{min-width:0}.official-document__footer{display:block}.official-document__footer span{display:block;margin-top:5px}}@media print{body{background:#fff!important}.official-document{box-shadow:none;border:0;margin:0;padding:18mm;width:210mm;min-height:297mm;page-break-after:always}.official-document__header{break-inside:avoid}.official-document__table{break-inside:avoid}.button,.contract-actions,nav,.official-print-toolbar{display:none!important}}`
  const printPageFix = '<style>@media print{.official-document{min-height:0!important;break-after:page!important;page-break-after:always!important}.official-document:last-child{break-after:auto!important;page-break-after:auto!important}}</style>'
  if (printMode) {
    // Standalone print document — no site chrome (header / footer / load curtain),
    // which is what left a blank first page in the generated PDF. Mirrors the
    // contract print route in src/pages/public/contractView.ts.
    const standaloneStyles = `html,body{margin:0;background:#fff}.official-document-standalone{max-width:900px;margin:0 auto;padding:24px 16px}.official-document-standalone .official-print-toolbar{display:flex;justify-content:flex-end;gap:12px;margin:0 auto 16px}.official-document-standalone .official-print-toolbar button,.official-document-standalone .official-print-toolbar a{padding:9px 14px;border:1px solid #c8d3dc;background:#172331;color:#fff;text-decoration:none;cursor:pointer;font:600 13px/1.2 Arial,sans-serif}.official-document-standalone .official-print-toolbar a{background:#fff;color:#172331}@media print{.official-document-standalone{max-width:none;margin:0;padding:0}.official-print-toolbar{display:none!important}.official-document{min-height:0!important;break-after:page!important;page-break-after:always!important}.official-document:last-child{break-after:auto!important;page-break-after:auto!important}}`
    return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex, nofollow"><title>打印发票与收据 - 电脑租赁管理系统</title><link rel="stylesheet" href="/styles.css"><style>${docStyles}</style><style>${standaloneStyles}</style></head><body class="official-document-standalone"><div class="official-document-page"><div class="official-print-toolbar"><button class="language-toggle" type="button" data-language-toggle data-language-label aria-label="切换到英文" title="切换到英文">English</button><button type="button" onclick="window.print()">打印凭证</button><a href="/orders/${order.id}/invoice">返回收据查看</a></div>${documents}</div><script src="/i18n.js" defer></script></body></html>`
  }
  const controls = printPageFix + `<div class="contract-actions" style="margin-bottom:20px"><a class="button button-primary" href="/orders/${order.id}/invoice/print" target="_blank" rel="noopener">打印 / 下载 PDF</a></div>`
