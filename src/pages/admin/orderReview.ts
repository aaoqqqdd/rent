...
export async function renderAdminOrderReview(c: Context, user: any) {
  const rows = ((await c.env.RENT.prepare(`
    SELECT o.id, o.orderNo, o.startDate, o.endDate, o.totalAmount, o.depositAmount, o.createdAt,
           o.deliveryMethod, o.rentalNote, u.name AS customerName, u.email AS customerEmail,
  ...
  const body = `<div class="page-header"><div><p class="section-code">ORDER REVIEW / WEBSITE</p><h2>网站订单审核</h2><p>数据直接来自 D1 的 orders 表，显示所有状态为“待审核”的订单。通过后进入合同与付款流程，拒绝后订单会取消。</p></div><a class="button button-secondary" href="/admin/orders">全部订单</a></div>
    ...
    <section class="panel"><div class="section-title"><h3>审核队列</h3><span class="section-note">请确认租期、设备档期和客户备注后处理</span></div>
      ${rows.length ? `<div class="table-wrapper"><table><thead><tr><th>订单 / 客户</th><th>设备</th><th>租期</th><th>金额</th><th>申请时间</th><th>操作</th></tr></thead><tbody>${rows.map(order => `<tr>
        ...
        <td><strong>${formatCurrency(order.totalAmount)}</strong><br><small>押金 ${formatCurrency(order.depositAmount)}</small></td>
        <td>${esc(formatMelbourneDateTime(order.createdAt))}</td>
        <td><div class="record-actions"><form method="post" action="/staff/orders/${encodeURIComponent(order.id)}/approve" data-site-confirm="确认通过这笔网站租赁申请吗？"><button class="button button-sm button-primary" type="submit">审核通过</button></form><form method="post" action="/staff/orders/${encodeURIComponent(order.id)}/reject" data-site-confirm="确认拒绝并取消这笔网站租赁申请吗？"><button class="button button-sm button-danger" type="submit">拒绝订单</button></form></div></td>
      </tr>${order.rentalNote ? `<tr><td colspan="6" class="section-note">客户备注：${esc(order.rentalNote)}</td></tr>` : ''}`).join('')}</tbody></table></div>` : '<div class="empty-state"><h3>暂无待审核订单</h3><p>网站新提交的租赁申请会自动出现在这里。</p></div>'}
    </section>`
