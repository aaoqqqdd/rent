...
export function renderAdminDataRetention(user: any, policies: any[] = [], preview: Record<string, number> = {}) {
  ...
  const rows = policies.map((p, index) => {
    ...
    const formId = `retention-policy-${index}`
    return `<tr>
      <td><strong>${esc(p.label, 80)}</strong><small>${esc(p.category, 60)}</small></td>
      <td class="retention-policy-copy">
        <form id="${formId}" method="post" action="/admin/data-retention/${encodeURIComponent(p.category)}">
      ...
      <td class="retention-policy-period"><input class="form-control" form="${formId}" aria-label="保留天数" type="number" name="retentionDays" min="0" max="36500" value="${Number(p.retention_days) || 0}" required><small>${years} 年</small></td>
      <td class="retention-policy-action"><select class="form-control" form="${formId}" aria-label="到期动作" name="action">${['RETAIN', 'ARCHIVE', 'DELETE', 'ANONYMISE'].map(a => `<option value="${a}" ${p.action === a ? 'selected' : ''}>${ACTION_LABELS[a]}</option>`).join('')}</select>${retentionSweepActionable(p) ? `<span class="badge badge-warning">${ACTION_LABELS[p.action]}</span>` : '<span class="badge badge-neutral">仅保留</span>'}</td>
      <td class="retention-policy-update"><span>${due == null ? '-' : `<strong>${due}</strong> 条已过期`}</span><small>${p.updated_by ? `上次更新：${esc(p.updated_by, 60)} · ${esc(p.updated_at, 40)}` : `${esc(p.updated_at, 40)}`}</small><div><label class="form-check"><input form="${formId}" type="checkbox" name="enabled" value="1" ${Number(p.enabled) === 1 ? 'checked' : ''}> 启用</label><button class="button button-sm button-primary" form="${formId}" type="submit">保存</button></div></td>
    </tr>`
  }).join('')
