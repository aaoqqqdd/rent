...
export function renderAdminDeviceControl(user: any, device: any, commands: any[] = [], maint?: MaintenanceView, successMessage?: string) {
  ...
  const body = `<div class="page-header"><div><p class="section-code">REMOTE CONTROL</p><h2>${esc(device.name)}</h2><p>${esc(device.model)} · ${esc(device.serialNumber || device.serial_number || '无序列号')} · 生命周期 <span class="badge badge-info">${esc(device.lifecycle_status || device.lifecycleStatus || 'READY')}</span></p></div><div class="record-actions">${(device.lifecycle_status === 'RETURNED' || device.lifecycle_status === 'INSPECTION' || device.lifecycleStatus === 'RETURNED' || device.lifecycleStatus === 'INSPECTION') ? `<a class="button button-primary" href="/admin/devices/${encodeURIComponent(device.id)}/accept">去验收</a>` : ''}<a class="button button-secondary" href="/admin/devices/${encodeURIComponent(device.id)}/edit">编辑设备</a><a class="button button-secondary" href="/admin/device-agent-bindings">绑定设备</a></div></div>` +
    (successMessage ? `<div class="alert alert-success" style="margin-bottom:16px">${esc(successMessage)}</div>` : '') +
