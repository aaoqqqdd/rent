  ...
  const body = `
    ...
    <div class="panel">
      <form method="POST" action="/admin/devices/${value('id')}/edit" class="asset-editor">
        ...
        <section class="form-section"><div class="form-section-title"><span class="mono">COMMANDS</span><div><h3>远程操作</h3><p>命令由设备客户端通过 HTTPS 领取并回传执行结果。</p></div></div><div class="record-actions"><button class="button button-secondary" name="commandType" value="SYNC" formmethod="post" formaction="/admin/devices/${value('id')}/commands" type="submit">立即同步</button><button class="button button-secondary" name="commandType" value="REFRESH_DEVICE_INFO" formmethod="post" formaction="/admin/devices/${value('id')}/commands" type="submit">刷新设备信息</button><button class="button button-warning" name="commandType" value="PAUSE_RENTAL" formmethod="post" formaction="/admin/devices/${value('id')}/commands" type="submit" data-site-confirm="确认暂停这台设备的租赁状态吗？">暂停设备</button><button class="button button-success" name="commandType" value="RESUME_RENTAL" formmethod="post" formaction="/admin/devices/${value('id')}/commands" type="submit">恢复设备</button></div><div class="grid grid-2" style="margin-top:12px"><input class="form-control" name="title" maxlength="120" placeholder="通知标题（仅发送通知时使用）"><input class="form-control" name="message" maxlength="500" placeholder="通知内容（SHOW_MESSAGE）"></div><button class="button button-secondary" name="commandType" value="SHOW_MESSAGE" formmethod="post" formaction="/admin/devices/${value('id')}/commands" type="submit" style="margin-top:10px">发送设备通知</button></section>
