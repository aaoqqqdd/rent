...
export function renderAdminDeviceEdit(user: any, device: any, commandHistory: any[] = []) {
  ...
  const lifecycleStatus = device.lifecycleStatus || device.lifecycle_status || (status === 'maintenance' ? 'MAINTENANCE' : status === 'retired' ? 'RETIRED' : status === 'rented' ? 'RENTED' : 'READY')
  const lifecycleEvents = Array.isArray(device.lifecycleEvents) ? device.lifecycleEvents : []
  const body = `
    <div class="page-header"><div><p class="section-code">ASSET RECORD</p><h2>编辑设备</h2><p>${value('name')} · <span class="mono">${value('assetTag', 'asset_tag') || value('id')}</span></p></div><div class="record-actions"><a href="/admin/devices/${value('id')}/control" class="button button-primary" data-full-navigation="true">远程控制</a><a href="/admin/device-agent-bindings" class="button button-secondary" data-full-navigation="true">Windows 客户端管理</a><a href="/admin/devices" class="button button-secondary" data-full-navigation="true">返回设备列表</a></div></div>
    <div class="panel">
      <form method="POST" action="/admin/devices/${value('id')}/edit" class="asset-editor">
        <section class="form-section"><div class="form-section-title"><span class="mono">ID</span><div><h3>设备身份</h3><p>更新后，员工端设备目录和搜索结果会使用这里的资料。</p></div></div>
          <div class="grid grid-2">
            ...
            <div class="form-group"><label class="form-label" for="lifecycleStatus">生命周期</label><select class="form-control" id="lifecycleStatus" name="lifecycleStatus"><option value="READY" ${lifecycleStatus === 'READY' ? 'selected' : ''}>可租用</option><option value="RESERVED" ${lifecycleStatus === 'RESERVED' ? 'selected' : ''}>已预约</option><option value="RENTED" ${lifecycleStatus === 'RENTED' ? 'selected' : ''}>租赁中</option><option value="RETURNED" ${lifecycleStatus === 'RETURNED' ? 'selected' : ''}>已归还待上架</option><option value="INSPECTION" ${lifecycleStatus === 'INSPECTION' ? 'selected' : ''}>归还验机中</option><option value="MAINTENANCE" ${lifecycleStatus === 'MAINTENANCE' ? 'selected' : ''}>维修中</option><option value="DAMAGED" ${lifecycleStatus === 'DAMAGED' ? 'selected' : ''}>损坏待处理</option><option value="RETIRED" ${lifecycleStatus === 'RETIRED' ? 'selected' : ''}>已退役</option></select><small class="form-text">变更会保留操作记录；“可租用”才会重新开放该设备。</small></div>
          </div>
        </section>
        <section class="form-section"><div class="form-section-title"><span class="mono">LIFE</span><div><h3>生命周期记录</h3><p>系统自动记录付款、交付、归还验机及管理员的状态调整。</p></div></div>${lifecycleEvents.length ? `<div class="table-wrapper"><table><thead><tr><th>时间</th><th>变更</th><th>原因</th></tr></thead><tbody>${lifecycleEvents.map((event: any) => `<tr><td>${esc(event.created_at || '-')}</td><td>${esc(event.previous_status || '—')} → ${esc(event.next_status || '—')}</td><td>${esc(event.reason || '—')}</td></tr>`).join('')}</tbody></table></div>` : '<p class="section-note">尚无生命周期变更记录。</p>'}</section>
        ...
        <section class="form-section"><div class="form-section-title"><span class="mono">AGENT</span><div><h3>Windows 设备代理</h3><p>查看代理注册和心跳信息；最后在线时间及设备信息由代理自动更新。</p></div></div>
          <div class="grid grid-2">
            ...
            <div class="form-group"><label class="form-label" for="deviceMode">客户设备状态</label><select class="form-control" id="deviceMode" name="deviceMode"><option value="normal" ${value('deviceMode', 'device_mode') === 'normal' ? 'selected' : ''}>正常使用</option><option value="return" ${value('deviceMode', 'device_mode') === 'return' ? 'selected' : ''}>等待归还验机</option><option value="maintenance" ${value('deviceMode', 'device_mode') === 'maintenance' ? 'selected' : ''}>维护中</option><option value="lost" ${value('deviceMode', 'device_mode') === 'lost' ? 'selected' : ''}>遗失 / 暂停使用</option></select><small class="form-text">保存后，Windows 客户端下一次同步会显示新的状态。</small></div>
            ...
            <div class="form-group"><label class="form-label">客户端版本</label><input class="form-control mono" value="${value('agentVersion', 'agent_version') || '未上报'}" readonly></div>
