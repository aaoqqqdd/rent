...
export async function renderAdminUserDetail(c: Context, user: any, targetUserId: string) {
  ...
  const body = `
    ...
    <div class="panel record-panel single-column"><div class="record-grid">
        ...
        <section class="record-section"><p class="section-code">FINANCE</p><h3>账户信息</h3><dl class="data-list"><div><dt>账户余额</dt><dd class="mono">AUD$${(targetUser.balance || 0).toFixed(2)}</dd></div><div><dt>佣金余额</dt><dd class="mono">AUD$${(targetUser.commissionBalance || 0).toFixed(2)}</dd></div>${targetUser.role === 'CUSTOMER' ? `<div><dt>BSB</dt><dd class="mono">${targetUser.bsb || '未填写'}</dd></div><div><dt>银行账号</dt><dd class="mono">${targetUser.accountNumber || targetUser.account_number || targetUser.account || '未填写'}</dd></div>` : ''}</dl>${pendingTopups.length ? `<div class="page-notification page-notification--info"><strong>待审核充值</strong>${pendingTopups.map((topup: any) => `<p>${topup.amount} AUD · Reference ${topup.reference || '-'} <form method="post" action="/admin/balance-topups/${topup.id}/approve" style="display:inline"><button class="button button-sm button-primary" type="submit">审核到账</button></form></p>`).join('')}</div>` : ''}${targetUser.role === 'CUSTOMER' ? `<form method="post" action="/admin/users/${targetUser.id}/balance-adjust" class="balance-adjust-form"><h4>调整账户余额</h4><label class="form-label" for="balanceAmount">变动金额（正数增加，负数减少）</label><input class="form-control" id="balanceAmount" name="amount" type="number" step="0.01" required><label class="form-label" for="balanceReason">调整原因（必填）</label><textarea class="form-control" id="balanceReason" name="reason" maxlength="300" required></textarea><button class="button button-primary" type="submit">保存余额调整</button></form>` : ''}</section>
      </div>
