...
export function renderAdminRiskFlags(user: any, targetUser: any, activeFlags: any[] = [], history: any[] = []) {
  ...
  const body = `<div class="page-header"><div><p class="section-code">RISK MANAGEMENT</p><h2>风险标记 · ${sanitizePlainText(targetUser.name, 100)}</h2><p>任意 HIGH 级标记或“拦截下单”类型（付款风险 / 设备未归还 / 拒付 / 疑似欺诈）会禁止客户自助下单；员工建合同不受此限制，但会看到提示。标记不会自动过期，只能由管理员手动解除。</p></div><a class="button button-secondary" href="/admin/users/${encodeURIComponent(targetUser.id)}">返回客户</a></div>
  <div class="panel">
    <h3>新增风险标记</h3>
