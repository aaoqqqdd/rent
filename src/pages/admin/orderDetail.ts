  ...
  const body = `
    ...
    <div class="panel">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">`: ''}
        ...
        ${canModifyOrder ? `<div style="padding: 24px; background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border-radius: 16px;">
        ...
        ${showRefundCard ? `<div style="padding: 24px; background: linear-gradient(135deg, #fef7ed 0%, #feedd9 100%); border-radius: 16px;">
          ...
          ${pendingBankTransferRefund ? `<div class="alert"><strong>${pendingBankTransferRefund.type === 'deposit' ? '押金退款待处理' : '订单已取消'}，需通过银行转账退还 ${formatCurrency(pendingBankTransferRefund.refund_amount)}</strong><br>BSB：${escapeHtml(pendingBankTransferRefund.refund_bsb || '未填写')}<br>账号：${escapeHtml(pendingBankTransferRefund.refund_account_number || '未填写')}<br>账户名：${escapeHtml(pendingBankTransferRefund.refund_account_name || '未填写')}</div>
          <form method="POST" action="/admin/refunds/${pendingBankTransferRefund.id}/complete-bank-transfer" onsubmit="return confirm('请确认已经完成银行转账，确认后系统将标记退款成功并通知客户。');"><button type="submit" class="button button-primary">确认已转账</button></form>` : ''}
