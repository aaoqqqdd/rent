  ...
  const body = `
    ...
    <div class="panel">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px;">
        ${['active', 'pending_return'].includes(String(order.status)) ? `<div style="padding:24px;background:#eff6ff;border-radius:16px"><h4>设备归还</h4><p>${String(order.status) === 'pending_return' ? '客户已申请提前归还，请完成归还验机。' : '订单租赁中，可进行归还验机并完成订单。'}</p><a class="button button-info" href="/staff/orders/${order.id}/inspection">归还验机</a></div>` : ''}
        ${order.status === 'completed' ? `<div style="padding:24px;background:#fff7ed;border-radius:16px;border:1px solid #fed7aa"><h4>清理设备资料</h4><p>确认设备已经归还并完成验机后，通知客户端清理非管理员账户的浏览器登录资料、微信登录资料、下载和桌面文件。</p><form method="POST" action="/admin/orders/${order.id}/confirm-device-cleanup" onsubmit="return confirm('确认设备已归还并清理所有非管理员用户资料吗？此操作不可恢复。');"><button type="submit" class="button button-warning">确认归还并清理设备</button></form></div>` : ''}
