...
export async function renderStaffInspection(c: Context, user: any, orderId: string) {
  ...
  const latestInspection = await c.env.RENT.prepare("SELECT snapshot_json, created_at FROM device_inspections WHERE device_id = ? AND inspection_type IN ('before_rental', 'automated_health') ORDER BY created_at DESC LIMIT 1").bind(order.deviceId).first() as any
  let automatic: any = {}
