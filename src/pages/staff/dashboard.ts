    ...
    <div class="panel">
      ...
      <table><thead><tr><th>订单号</th><th>客户</th><th>设备</th><th>状态</th><th>操作</th></tr></thead><tbody>
        ${orders.slice(0, 5).map((order) => {
          ...
          return `<tr><td>${order.orderNo || order.order_no || 'N/A'}</td><td>${customer?.name ?? '未知用户'}</td><td>${device?.name ?? '未知设备'}</td><td><span class="badge ${statusClass}">${order.status}</span></td><td><a class="link-button" href="/staff/orders/${order.id}">查看详情</a></td></tr>`
        }).join('')}
      </tbody></table>
    </div>
    <div class="panel">
      ...
      <table><thead><tr><th>设备名称</th><th>状态</th><th>当前租用者</th><th>操作</th></tr></thead><tbody>
        ${devices.slice(0, 5).map((device) => {
          ...
          return `<tr><td>${device.name}</td><td><span class="badge ${deviceStatusClass}">${device.status}</span></td><td>${customer?.name ?? '无'}</td><td><a class="link-button" href="/staff/devices/${device.id}">查看详情</a></td></tr>`
        }).join('')}
      </tbody></table>
    </div>
  `
