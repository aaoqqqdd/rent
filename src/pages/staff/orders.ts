...
import { buildLayout, getOrders, getUsers, getDevices, formatCurrency, staffOrderPath } from '../../site';
...
export async function renderStaffOrders(c: Context, user: any, searchTerm: string = '') {
  ...
  const body = `
    <div class="panel">
      <div class="section-title"><p class="section-code">RENTAL OPERATIONS</p><h2>租赁管理</h2><span class="section-note">管理租赁订单、交付状态、租期和归还流程；合同内容请前往合同管理。</span></div>
      ...
      <div class="table-wrapper">
        <table class="table"><thead><tr><th>订单号</th><th>客户</th><th>设备</th><th>租期</th><th>金额</th><th>状态</th><th>操作</th></tr></thead><tbody>
          ${filteredOrders.map((order) => {
            const customer = usersData.find(u => u.id === order.userId);
            const device = devicesData.find(d => d.id === order.deviceId);
            return `<tr><td>${order.orderNo || '<span class="text-muted">付款后生成</span>'}</td><td>${customer?.name ?? 'N/A'}${customer?.accountType === 'guest' ? ' <span class="badge badge-warning">访客/临时账户</span>' : ''}</td><td>${device?.name ?? 'N/A'}</td><td>${order.startDate} ~ ${order.endDate}</td><td>${formatCurrency(order.totalAmount)}</td><td>${order.status}</td><td><a class="link-button" href="${staffOrderPath(order)}">查看</a></td></tr>`;
          }).join('')}
        </tbody></table>
      </div>
    </div>
  `;
  return buildLayout('租赁管理 - 电脑租赁管理系统', body, user);
}
