...
export async function renderCustomerOrderDetail(c: Context, user: any, orderId: string, message?: string, type: 'success' | 'error' = 'error') {
  ...
  const body = `
    <div class="panel">
      ...
      ${['paid', 'active', 'completed'].includes(order.status) || String(order.status) === 'pending_return' ? `<p><a class="button button-secondary" href="/orders/${order.id}/invoice">查看发票</a>${order.status === 'active' ? `<form method="post" action="/customer/orders/${order.id}/early-return" style="display:inline-block;margin-left:10px" onsubmit="return confirm('确定申请提前归还此设备吗？提交后请等待工作人员验机。')"><button class="button button-warning" type="submit">申请提前归还</button></form>` : String(order.status) === 'pending_return' ? ' <span class="badge badge-warning">等待归还验机</span>' : ''}</p>` : ''}
      ${contract ? `
