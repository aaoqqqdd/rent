...
const PAYMENT_METHOD_LABELS: Record<string, string> = {
  stripe: '信用卡（Stripe）', card: '信用卡（Stripe）', square: 'Square 礼品卡', bank_transfer: '银行转账', alipay: '支付宝', wechat: '微信',
  ...
  const body = `
    <div class="panel order-detail-shell customer-order-detail">
      ...
      ${(paymentSources.length || refundRows.length) ? `<section class="panel" style="margin-top:20px"><h3>付款与退款明细</h3>
        ${paymentSources.length ? `<h4 style="margin:12px 0 6px">付款 / 预授权</h4><dl class="data-list">${paymentSources.map((p: any) => `<div><dt>${esc(p.payment_provider === 'square' ? 'Square 礼品卡' : PAYMENT_METHOD_LABELS[String(p.payment_method)] || p.payment_method)}${p.status === 'pending' ? '（待结算）' : ''}</dt><dd>${formatCurrency(p.amount)}</dd></div>`).join('')}</dl>` : ''}
      ...
      ${order.status === 'pending_payment' ? `
        ...
        <div class="payment-options" style="display: flex; gap: 20px; margin-top: 16px;">
          ...
          ${squarePaymentSelected && systemSettings.paymentMethods.square ? `<div class="payment-card">
            <h4>Square 礼品卡支付</h4>
            <p>礼品卡仅支付租金及服务费 ${formatCurrency(immediatelyPaidAmount)}；押金按订单约定单独处理。</p>
            ${renderSquareGiftCardPaymentBox({ configUrl: `/customer/orders/${order.id}/square/config`, paymentUrl: `/customer/orders/${order.id}/square/payment`, returnUrl: `/payment/result?orderId=${encodeURIComponent(order.id)}`, buttonLabel: `使用 Square 礼品卡支付 ${formatCurrency(immediatelyPaidAmount)}`, domId: 'order-square-gift-card-pay' })}
          </div>` : ''}
          ${!squarePaymentSelected && systemSettings.paymentMethods.stripe ? `<div class="payment-card">
            <h4>信用卡支付（Stripe）</h4>
        ...
        ${!squarePaymentSelected && ['card', 'stripe'].includes(String(order.paymentMethod)) ? (() => {
          ...
          if (systemSettings.paymentMethods.square && !squarePaymentSelected) alternatives.push({ value: 'square', label: 'Square 礼品卡' })
