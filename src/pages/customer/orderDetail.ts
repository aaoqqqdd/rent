...
export async function renderCustomerOrderDetail(c: Context, user: any, orderId: string, message?: string, type: 'success' | 'error' = 'error') {
  ...
  const body = `
    <div class="panel order-detail-shell customer-order-detail">
      ...
      <section class="panel" style="margin-top:20px"><h3>出租前验机报告</h3><p class="form-text">该报告为设备交付前的验机记录。</p>${preInspection ? `${report(preSnapshot, ['screen','keyboard','touchpad','body','camera','wifi','power'])}<small class="form-text">记录时间：${esc(preInspection.created_at)}</small>` : '<p>暂无出租前验机记录。</p>'}</section>
