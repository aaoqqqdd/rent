...
import { buildLayout, getOrderById, getDeviceById, formatCurrency, formatMelbourneDateTime, getContractByOrderId, ensureContractForOrder, getCustomerRiskAssessment, systemSettings, diffOrderSnapshots, ORDER_CHANGE_TYPE_LABELS } from '../../site';
...
export async function renderCustomerOrderDetail(c: Context, user: any, orderId: string, message?: string, type: 'success' | 'error' = 'error') {
  ...
  const [device, existingContract, transferProof, timeChanges, preInspection, returnInspection, depositRefund, refundStatus, changeHistory, paymentSources, refundRows, priceAdjustmentSummary, priceAdjustmentProof, account] = await Promise.all([getDeviceById(c, order.deviceId), getContractByOrderId(c, order.id), (['bank_transfer', 'alipay', 'wechat'].includes(String(order.paymentMethod)) || String(order.paymentProvider || order.payment_provider || '') === 'square') ? c.env.RENT.prepare("SELECT pp.status, pp.reference_number, pp.rejection_reason, pp.note FROM payment_proofs pp JOIN payments p ON p.id = pp.payment_id WHERE p.rental_id = ? AND p.id NOT IN (SELECT COALESCE(payment_id, '') FROM order_price_adjustments WHERE order_id = ?) ORDER BY pp.uploaded_at DESC LIMIT 1").bind(order.id, order.id).first() : Promise.resolve(null), c.env.RENT.prepare('SELECT * FROM order_time_change_history WHERE order_id = ? ORDER BY created_at DESC LIMIT 10').bind(order.id).all(), c.env.RENT.prepare("SELECT snapshot_json, created_at FROM device_inspections WHERE rental_id = ? AND inspection_type = 'before_rental' ORDER BY created_at DESC LIMIT 1").bind(order.id).first(), c.env.RENT.prepare("SELECT snapshot_json, created_at FROM device_inspections WHERE rental_id = ? AND inspection_type = 'after_return' ORDER BY created_at DESC LIMIT 1").bind(order.id).first(), c.env.RENT.prepare("SELECT deduction_amount, deduction_reason FROM payment_refunds WHERE order_id = ? AND type = 'deposit' AND status = 'succeeded' ORDER BY created_at DESC LIMIT 1").bind(order.id).first(), c.env.RENT.prepare("SELECT status, refundable_amount, refund_amount FROM payment_refunds WHERE order_id = ? ORDER BY created_at DESC LIMIT 1").bind(order.id).first(), c.env.RENT.prepare("SELECT change_type, before_json, after_json, created_at FROM order_change_history WHERE order_id = ? ORDER BY created_at DESC LIMIT 20").bind(order.id).all(), c.env.RENT.prepare("SELECT payment_method, payment_provider, amount, deposit_amount, rental_amount, processing_fee, status, square_paid_amount FROM payments WHERE rental_id = ? AND status IN ('pending','paid','refunded') ORDER BY created_at").bind(order.id).all().then((r: any) => (r.results || []) as any[]), c.env.RENT.prepare("SELECT type, refund_amount, refund_method, status, created_at FROM payment_refunds WHERE order_id = ? AND status = 'succeeded' ORDER BY created_at").bind(order.id).all().then((r: any) => (r.results || []) as any[]), getOrderPriceAdjustmentSummary(c, order), c.env.RENT.prepare("SELECT pp.status, pp.reference_number, pp.rejection_reason FROM payment_proofs pp JOIN payments p ON p.id = pp.payment_id JOIN order_price_adjustments a ON a.payment_id = p.id WHERE a.order_id = ? AND a.direction = 'increase' ORDER BY pp.uploaded_at DESC LIMIT 1").bind(order.id).first(), c.env.RENT.prepare('SELECT balance FROM users WHERE id = ?').bind(user.id).first()]) as any[]
  const shouldEnsureContract = ['approved', 'paid', 'pending_pickup', 'active', 'extended', 'overdue', 'suspended', 'pending_return', 'returned', 'completed'].includes(String(order.status))
  const risk = !existingContract && shouldEnsureContract ? await getCustomerRiskAssessment(c, user.id) : null
  if (risk?.blocked) return buildLayout('订单暂不可继续 - 电脑租赁管理系统', `<div class="panel"><h2>订单暂不可继续</h2><p>当前账户存在风控限制，系统不会为该订单创建新的合同。请联系客服处理。</p></div>`, user)
  const contract = existingContract || (shouldEnsureContract ? await ensureContractForOrder(c, order, user.id) : null)
  ...
  const stripeTotal = depositMode === 'PREAUTH' ? Number(order.totalAmount) + stripeFee : Number(order.totalAmount) - deposit + stripeFee
  const squareFeeRate = Math.min(1, Math.max(0, Number(systemSettings.paymentMethods.squareProcessingFeeRate ?? 0.022)))
  const squareFee = Math.round(immediatelyPaidAmount * 100 * squareFeeRate) / 100
  ...
  const renderAdjustmentTransferCard = (method: 'bank_transfer' | 'alipay' | 'wechat') => {
    const label = method === 'bank_transfer' ? '银行转账' : method === 'alipay' ? '支付宝' : '微信'
    const paymentDetails = method === 'bank_transfer'
      ? `<p><strong>银行名称:</strong> ${systemSettings.bankDetails.bankName || '—'}</p><p><strong>BSB:</strong> ${systemSettings.bankDetails.bsb}</p><p><strong>账号:</strong> ${systemSettings.bankDetails.account}</p>`
      : `<img src="${method === 'alipay' ? systemSettings.rmbPayment.alipayQrUrl : systemSettings.rmbPayment.wechatQrUrl}" alt="${label}收款码" loading="lazy" style="max-width:240px;display:block;margin:12px 0">`
    const summaryId = `rmb-adjustment-summary-${method}`
    return `<div class="payment-card"><h4>${label}补差价</h4>${paymentDetails}<p id="${summaryId}">请支付 ${formatCurrency(priceAdjustmentSummary.amountDue)}${method === 'bank_transfer' ? '' : '；提交付款凭证前获取实时汇率。'}</p><form method="POST" action="/customer/orders/${order.id}/bank-transfer-proof"><input type="hidden" name="priceAdjustment" value="1"><input type="hidden" name="paymentMethod" value="${method}"><label class="form-label">付款 Reference</label><input class="form-control" name="referenceNumber" maxlength="100" required><label class="form-label">付款凭证图片链接</label><input class="form-control" type="url" name="imageUrl" placeholder="https://..." required><label class="form-label">备注（选填）</label><textarea class="form-control" name="note" maxlength="500"></textarea><button class="button button-primary" type="submit" style="margin-top:12px">提交${label}差价凭证</button></form>${method !== 'bank_transfer' ? `<script>(()=>{const s=document.getElementById('${summaryId}');fetch('/api/payment/aud-cny?amount=${encodeURIComponent(String(priceAdjustmentSummary.amountDue))}').then(r=>r.ok?r.json():Promise.reject()).then(d=>{s.innerHTML='请支付 <strong>CNY '+Number(d.cnyAmount).toFixed(2)+'</strong>，1 AUD = '+Number(d.rate).toFixed(6)+' CNY，金额按两位小数上舍入。'}).catch(()=>{s.textContent='暂时无法获取实时人民币汇率，请稍后重试。'})})()</script>` : ''}</div>`
  }
    const renderSquareAdjustmentCard = () => `<div class="payment-card"><h4>礼品卡补差价</h4><p>差价 ${formatCurrency(priceAdjustmentSummary.amountDue)}；礼品卡手续费（${(squareFeeRate * 100).toFixed(2)}%）${formatCurrency(squareAdjustmentFee)}；实际扣款 ${formatCurrency(squareAdjustmentTotal)}。退款时仅退回账户余额。</p>${renderSquareGiftCardPaymentBox({ configUrl: `/customer/orders/${order.id}/price-adjustment/square/config`, paymentUrl: `/customer/orders/${order.id}/price-adjustment/square/payment`, returnUrl: `/payment/result?orderId=${encodeURIComponent(order.id)}`, buttonLabel: `使用礼品卡支付 ${formatCurrency(squareAdjustmentTotal)}`, domId: 'order-price-adjustment-square-pay', stored: Boolean((order as any).square_gift_card_id || (order as any).squareGiftCardId) })}</div>`
  const balanceAlternativePayments = priceAdjustmentProof?.status === 'submitted'
    ? '<div class="payment-waiting-note"><span>差价付款凭证已提交，正在等待管理员审核</span></div>'
    : `${systemSettings.paymentMethods.square ? renderSquareAdjustmentCard() : ''}${systemSettings.paymentMethods.stripe ? `<div class="payment-card"><h4>信用卡支付（Stripe）</h4><p>使用其他支付方式补交差价，卡号由 Stripe 安全处理。</p>${renderStripePaymentBox({ intentUrl: `/customer/orders/${order.id}/price-adjustment/stripe/intent`, returnUrl: `/payment/result?orderId=${encodeURIComponent(order.id)}`, buttonLabel: `支付差价 ${formatCurrency(priceAdjustmentSummary.chargedAmount)}`, domId: 'order-price-adjustment-balance-stripe-pay' })}</div>` : ''}${systemSettings.paymentMethods.bankTransfer ? renderAdjustmentTransferCard('bank_transfer') : ''}${systemSettings.paymentMethods.alipay && systemSettings.rmbPayment.alipayQrUrl ? renderAdjustmentTransferCard('alipay') : ''}${systemSettings.paymentMethods.wechat && systemSettings.rmbPayment.wechatQrUrl ? renderAdjustmentTransferCard('wechat') : ''}`
  const squareAdjustmentPayment = systemSettings.paymentMethods.square && priceAdjustmentProof?.status !== 'submitted' ? renderSquareAdjustmentCard() : ''
  const currentTransferMethod = String(order.paymentMethod) as 'bank_transfer' | 'alipay' | 'wechat'
  const transferAdjustmentPayment = priceAdjustmentProof?.status === 'submitted'
    ? '<div class="payment-waiting-note"><span>差价付款凭证已提交，正在等待管理员审核</span></div>'
    : `${squareAdjustmentPayment}${renderAdjustmentTransferCard(currentTransferMethod)}`
  const cardAdjustmentPayment = `${squareAdjustmentPayment}${systemSettings.paymentMethods.stripe ? `<div class="payment-card"><p>在本页安全填写卡信息完成差价支付，卡号由 Stripe 处理。</p>${renderStripePaymentBox({ intentUrl: `/customer/orders/${order.id}/price-adjustment/stripe/intent`, returnUrl: `/payment/result?orderId=${encodeURIComponent(order.id)}`, buttonLabel: `支付差价 ${formatCurrency(priceAdjustmentSummary.chargedAmount)}`, domId: 'order-price-adjustment-stripe-pay' })}</div>` : '<p>信用卡支付当前未启用，请联系客服完成差价支付。</p>'}`
  const priceAdjustmentPaymentContent = priceAdjustmentSummary.amountDue <= 0
    ? ''
    : order.paymentMethod === 'balance'
      ? balanceAlternativePayments || '<p>当前没有可用的其他支付方式，请联系客服补交差价。</p>'
      : transferPayment
        ? transferAdjustmentPayment
        : cardAdjustmentPayment
  ...
  const body = `
    <div class="panel order-detail-shell customer-order-detail">
      ...
      ` : ''}
      ${squareDepositPending ? `<section class="panel" style="margin-top:20px"><h3>押金支付</h3>${systemSettings.paymentMethods.stripe && transferProof?.status !== 'submitted' ? `<div class="payment-card"><h4>Stripe 支付押金</h4>${renderStripePaymentBox({ intentUrl: `/customer/orders/${order.id}/stripe/deposit-intent`, returnUrl: `/customer/orders/${order.id}`, buttonLabel: `使用 Stripe 支付押金 ${formatCurrency(deposit)}`, domId: 'order-square-stripe-deposit-pay' })}</div>` : ''}<div class="payment-card"><h4>银行转账支付押金</h4><p><strong>银行名称:</strong> ${systemSettings.bankDetails.bankName || '—'}<br><strong>BSB:</strong> ${systemSettings.bankDetails.bsb}<br><strong>账号:</strong> ${systemSettings.bankDetails.account}</p>${transferProof?.status === 'submitted' ? '<div class="payment-waiting-note"><span>押金转账凭证已提交，正在等待管理员审核。</span></div>' : transferProof?.status === 'rejected' ? `<div class="alert">押金转账凭证未通过审核：${esc(transferProof.rejection_reason || '请重新提交正确的凭证。')}</div>` : `<form method="POST" action="/customer/orders/${order.id}/bank-transfer-proof"><input type="hidden" name="depositProof" value="1"><label class="form-label" for="squareDepositReference">银行 Reference</label><input class="form-control" id="squareDepositReference" name="referenceNumber" maxlength="100" required><label class="form-label" for="squareDepositProofImage">转账凭证图片链接</label><input class="form-control" type="url" id="squareDepositProofImage" name="imageUrl" placeholder="https://图床域名/凭证图片.jpg" required><small class="form-text">请先上传到图床，再粘贴公开 HTTPS 图片链接。</small><label class="form-label" for="squareDepositNote">备注（选填）</label><textarea class="form-control" id="squareDepositNote" name="note" maxlength="500"></textarea><button class="button button-secondary" type="submit" style="margin-top:12px">提交押金转账凭证</button></form>`}</div></section>` : ''}
      ${(priceAdjustmentSummary.amountDue > 0 || priceAdjustmentSummary.pendingDepositRefund > 0) && !['pending_payment', 'completed', 'cancelled'].includes(String(order.status)) ? `<section class="panel" style="margin-top:20px"><h3>订单差价</h3>${priceAdjustmentSummary.pendingDepositRefund > 0 ? `<div class="alert"><strong>订单已下调，待退差价 ${formatCurrency(priceAdjustmentSummary.pendingDepositRefund)}。</strong>由于您使用${order.paymentMethod === 'bank_transfer' ? '银行转账' : order.paymentMethod === 'alipay' ? '支付宝' : '微信'}付款，差价将在管理员退押金时一并退还。</div>` : ''}${priceAdjustmentSummary.amountDue > 0 ? `<div class="alert"><strong>订单价格已增加，需要补交 ${formatCurrency(priceAdjustmentSummary.amountDue)}。</strong>${order.paymentMethod === 'balance' ? `余额支付后当前余额 ${formatCurrency(currentBalance)}，请使用下方其他支付方式补交差价。` : transferPayment ? '转账类付款请提交差价凭证，管理员审核后生效。' : squarePaymentSelected ? '礼品卡补差价不收 Stripe 手续费；退款时仅退回账户余额。' : `信用卡支付含手续费 ${formatCurrency(priceAdjustmentSummary.processingFee)}，本次实际扣款 ${formatCurrency(priceAdjustmentSummary.chargedAmount)}。`}</div>` : ''}
        ${priceAdjustmentPaymentContent}
      ...
      <script>(function(){window.addEventListener('square-payment-updated',function(event){var detail=event.detail||{};var node=document.getElementById('square-payment-summary');if(!node)return;var paid=Number(detail.squarePaidAmountCents||0)/100;var remaining=Number(detail.remainingAmountCents||0)/100;node.hidden=paid<=0;node.innerHTML='<strong>礼品卡已扣:</strong> AUD$ '+paid.toFixed(2)+(remaining>0?'；Stripe 剩余 AUD$ '+remaining.toFixed(2):'');});})();</script>
    </div>
  `
  return buildLayout('订单详情 - 电脑租赁管理系统', body.replace('礼品卡补差价不收 Stripe 手续费；退款时仅退回账户余额。', `礼品卡补差价含 ${(squareFeeRate * 100).toFixed(2)}% 手续费；退款时仅退回账户余额。`), user)
}
