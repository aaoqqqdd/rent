...
export function deliveryStatusInfo(value: unknown): DeliveryStatusInfo {
  ...
  const info = STATUS_MAP[statusKey(raw)]
  return info || { key: raw || 'Unknown', label: raw || 'Unknown', description: '配送商已更新订单状态。', special: false }
}

export function deliveryStatusLabel(value: unknown): string {
  return deliveryStatusInfo(value).label
}

export function deliveryStatusDescription(value: unknown): string {
  return deliveryStatusInfo(value).description
}
