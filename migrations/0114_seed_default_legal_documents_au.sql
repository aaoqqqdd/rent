...
------------------------------------------------------------------------------
-- 9. 正式租赁合同模板（签署后生成的实际合同，支持合同变量）
--    表结构见 0008；0008 曾植入 {{customerName}} 风格的占位模板。
--    此处仅在模板「为空」或「仍是 0008 占位文案」时升级，绝不覆盖管理员编辑。
------------------------------------------------------------------------------
INSERT INTO contract_templates (id, name, content) VALUES ('default', '标准租赁合同模板',
'<h1>设备租赁合同</h1>
...
ON CONFLICT(id) DO UPDATE SET name = excluded.name, content = excluded.content, updatedAt = CURRENT_TIMESTAMP
WHERE contract_templates.content IS NULL
   OR TRIM(contract_templates.content) = ''
   OR contract_templates.content LIKE '%{{customerName}}%';
