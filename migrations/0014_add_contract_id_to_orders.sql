...
-- We'll link based on the rentalId (which is now orderId in contracts table)
UPDATE orders
SET contractId = (SELECT id FROM contracts WHERE contracts.rentalId = orders.id LIMIT 1)
WHERE id IN (SELECT rentalId FROM contracts);
