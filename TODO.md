...
* 风险标记与黑名单（= TODO #10）✅：CRUD / 8 类型 / MANAGER+ / 审计此前已具备。site.ts 提供 isRiskFlagCurrentlyActive / findBlockingRiskFlag + ORDER_BLOCKING_RISK_FLAG_TYPES（PAYMENT_RISK / DEVICE_NOT_RETURNED / CHARGEBACK / FRAUD_SUSPECTED）——自助下单拦截为「任意 HIGH 级 或 硬拦截类型」（下单校验、风险页有效列表、员工 / 管理员客户页风险提示，均按严重程度排序）。风险标记不会自动过期，只能由管理员在风险页手动解除（写 RISK_FLAG_RESOLVED 审计）；已移除创建表单和展示逻辑中的 expires_at 字段。风险页展示「拦截下单」徽标。测试 tests/riskFlags.test.ts。
